...
def run_scrape(url, media_types, min_size_kb=0, max_depth=2, max_pages=100,
               same_domain=True, proxy=None, bypass_choice="1",
               use_external_viewer=False, max_per_type=None, save_dir=None):
    """Run the existing scrape pipeline using the supplied parameters."""
    global visited_urls, downloaded_media, download_queue
...
class Duke2GUIApp(App):
    """Minimal Android-friendly GUI for selecting a single media type and URL."""

    def build(self):
        if not KIVY_AVAILABLE:
            return Label(text="Kivy is required for GUI mode.")

        self.title = "Duke2 Enhanced"

        root = BoxLayout(orientation='vertical', padding=20, spacing=12)
        root.add_widget(Label(text='Duke2 Enhanced', font_size=24, bold=True, color=(0.95, 0.22, 0.35, 1), size_hint_y=None, height=40))
        root.add_widget(Label(text='Enter a URL and choose the media type to download.', size_hint_y=None, height=24))

        self.url_input = TextInput(hint_text='https://example.com', multiline=False, size_hint_y=None, height=48)
        root.add_widget(self.url_input)

        root.add_widget(Label(text='Media type', size_hint_y=None, height=24))
        self.media_spinner = Spinner(
            text='images',
            values=['images', 'videos', 'audio', 'documents', 'archives', 'ebooks', 'all'],
            size_hint_y=None,
            height=48,
        )
        root.add_widget(self.media_spinner)

        self.status_label = Label(text='Status: Ready', size_hint_y=None, height=80, halign='center', valign='middle', text_size=(400, None))
        root.add_widget(self.status_label)

        start_button = Button(text='Start Download', size_hint_y=None, height=56)
    ...
    def start_download(self, instance):
        ...
        if not url:
            self.set_status('Please enter a URL first.')
            return
        if not url.startswith(('http://', 'https://')):
            url = 'https://' + url

        selected = self.media_spinner.text
        media_types = list(MEDIA_EXTENSIONS.keys()) if selected == 'all' else [selected]
        self.set_status(f'Starting {selected} download...')
        threading.Thread(target=self._run_download, args=(url, media_types), daemon=True).start()

    def _run_download(self, url, media_types):
        try:
            result = run_scrape(url, media_types, save_dir='/sdcard/Download/Duke2')
            self.set_status('Download complete. Check your download folder.')
...
def main():
    ...
    gallery_path = run_scrape(
        ...
        proxy=proxy,
        bypass_choice=bypass_choice,
        use_external_viewer=use_external_viewer,
